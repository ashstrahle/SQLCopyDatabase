...
GO
	...
	--
	-- ==========================================================================================
	CREATE PROCEDURE [dbo].[sp_CopyDatabase](
		@src_database NVARCHAR(MAX),
		@tgt_database NVARCHAR(MAX)
	) AS BEGIN -- SET NOCOUNT ON added to prevent extra result sets from
	-- interfering with SELECT statements.
SET
	NOCOUNT ON;

DECLARE @src_latestbackupfile AS NVARCHAR(MAX),
...
SELECT
	TOP 1 @src_latestbackupfile = bmf.physical_device_name
FROM
	msdb.dbo.backupmediafamily bmf
	LEFT JOIN msdb.dbo.backupset bs ON bs.media_set_id = bmf.media_set_id
WHERE
	bs.database_name = @src_database
...
SELECT
	@src_mdb_name = mf.name
FROM
	sys.master_files mf
WHERE
	DB_ID(@src_database) = mf.database_id
	AND mf.type_desc = 'ROWS';

-- Get the target database path
SELECT
	@tgt_mdb_path = mf.physical_name
FROM
	sys.master_files mf
WHERE
	mf.database_id = DB_ID(@tgt_database)
	AND mf.type_desc = 'ROWS';

-- Get the source database log logical name
SELECT
	@src_log_name = mf.name
FROM
	sys.master_files mf
WHERE
	mf.database_id = DB_ID(@src_database)
	AND mf.type_desc = 'LOG';

-- Get the target database log path
SELECT
	@tgt_log_path = mf.physical_name
FROM
	sys.master_files mf
WHERE
	mf.database_id = DB_ID(@tgt_database)
...
WHERE
	database_id = DB_ID(@tgt_database) EXEC(@kill);

-- Kick off restore
RESTORE DATABASE @tgt_database
FROM
	disk = @src_latestbackupfile WITH REPLACE,
